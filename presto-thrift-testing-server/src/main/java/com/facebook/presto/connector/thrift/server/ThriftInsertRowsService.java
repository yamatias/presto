        ...
        implements PrestoThriftService
{
    ...
    // TODO: create a separate wrapper for a ThriftPage that doesn't have a token.
    // TODO: You will need to return a token in addRows so that presto can continue to serve rows to the system.
    private static final List<PrestoThriftPageResult> TESTTABLE = new ArrayList<>();
    // TODO: allow for tag column names
    private static final List<String> COLUMNNAMES = ImmutableList.of("value", "x", "y", "zoom_level");
    private static final List<Type> COLUMNTYPES = ImmutableList.of(DOUBLE, INTEGER, INTEGER, INTEGER);
    ...
    public PrestoThriftNullableTableMetadata getTableMetadata(PrestoThriftSchemaTableName schemaTableName)
    {
        ...
        return new PrestoThriftNullableTableMetadata(new PrestoThriftTableMetadata(schemaTableName, columnMetadata, null, null));
    }
    ...
    private PrestoThriftPageResult getRowsSync(PrestoThriftId splitId, List<String> columns, long maxBytes, PrestoThriftNullableToken nextToken)
    {
        ...
        int rowCount = 0;
        for (PrestoThriftPageResult pageResult : TESTTABLE) {
